...
def main():
    ...
    if args.subpackage == 's3':
        ...
        extractedData,start_date,end_date = clickhouse_loader.connect_clickhouse(args)
    ...
    # Save
    if args.subpackage == 'clickhouse':
        #extractedData.to_csv("Data/TotalCO2Data_Clickhouse/extractedData_" + str(args.subpackage) + "_" + str(args.query) + "_" + start_date + "_to_" + end_date + ".csv", index=False )
        extractedData.to_csv("extractedData_" + str(args.subpackage) + "_" + str(args.query) + ".csv", index=False )  # TODO: sep=','
    else:
